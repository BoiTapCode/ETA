...
def pre_process(fin, fout, city_name=None, is_test=False, thread_num = 20):
    print('Raw input file:' + fin)
    print('Temporary file:' + fout)
    df = pd.read_csv(fin)
    print('length of the original data: ', len(df))

    # Tag rows with an explicit city identifier so downstream code (dataset_delivery.py)
    # can split / group by city without relying on courier_id or region_id conventions
    # that may not be consistent across raw files.
    df['city'] = city_name if city_name is not None else fin
    ...
    # City-centered coordinates: absolute lat/lng do not transfer across cities
    # (different cities occupy completely different coordinate ranges), so a model
    # trained on raw lat/lng will not generalize to an unseen city. Center the
    # coordinates on this city's own centroid; downstream (dataset_delivery.py)
    # should use these columns instead of raw 'lat'/'lng' as model input features.
    # Raw 'lat'/'lng' are kept untouched since geodesic distance calculations need them.
    lng_center, lat_center = df['lng'].mean(), df['lat'].mean()
    ...
    cou_df['city'] = df['city'].iloc[0] if 'city' in df.columns else city_name
