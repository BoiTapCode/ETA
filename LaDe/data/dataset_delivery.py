...
class DeliveryDataset(object):
    ...
    def get_features(self, cou, mode):
        ...
        # self.aoi_dict / self.aoi_feature are keyed by city (see get_data()); a whole
        # trajectory always belongs to a single city, so resolve the right table once here.
        city = cou['city'].iloc[0] if 'city' in cou.columns else None
        aoi_dict = self.aoi_dict[city] if isinstance(self.aoi_dict, dict) and city in self.aoi_dict else self.aoi_dict
        aoi_feature = self.aoi_feature[city] if isinstance(self.aoi_feature, dict) and city in self.aoi_feature else self.aoi_feature
        ...
        for start in range(len(c_v)):  # time steps are decided by finish time
            ...
            now_lng = c_v[start][self.idx_longitude_raw]
            now_lat = c_v[start][self.idx_latitude_raw]
            aoi_start_idx = aoi_dict[c_v[start][self.idx_aoi]]
            aoi_start_feature = aoi_feature[aoi_start_idx][:4]
            ...
            if len(todo_rank) <= self.N_min:
                ...
                last_aoi = aoi_dict[cou[cou['index'] == todo_rank[0]]['aoi_id'].iloc[0]]  # aoi index of the first unfinished task
                last_aoi_feature = aoi_feature[last_aoi]
                aoi_set = {last_aoi}
                aoi_sum = 1
                aoi_fea = []
                ...
                for i in range(1, len(todo_rank)):
                    aoi = aoi_dict[cou[cou['index'] == todo_rank[i]]['aoi_id'].iloc[0]]
                    aoi_type = aoi_feature[aoi][1]
                    if aoi == last_aoi:
                        aoi_sum += 1
                        if i == len(todo_rank) - 1:
                            aoi_lng = aoi_feature[aoi][2]
                            aoi_lat = aoi_feature[aoi][3]
                    ...
                    aoi_lng = aoi_feature[last_aoi][2]
                    aoi_lat = aoi_feature[last_aoi][3]
                    aoi_dis = self.dis_cal(now_lat, now_lng, aoi_lat, aoi_lng)
                    last_aoi = aoi
                    last_aoi_feature = aoi_feature[last_aoi]
                    aoi_set.add(aoi)
            ...
            for task in todo_lst:
                ...
                dis_abs = self.dis_cal(c_v[start][self.idx_latitude_raw], c_v[start][self.idx_longitude_raw],
                                  c_v[c_idx][self.idx_latitude_raw], c_v[c_idx][self.idx_longitude_raw])
                ...
                aoi_node_feature[t, idx, :] = aoi_feature[aoi_dict[c_v[c_idx][self.idx_aoi]]]
        ...
        for i in sample_valid_task:
            for j in sample_valid_task:
                ...
                dis_abs = self.dis_cal(c_v[c_idx_i][self.idx_latitude_raw], c_v[c_idx_i][self.idx_longitude_raw],
                                  c_v[c_idx_j][self.idx_latitude_raw], c_v[c_idx_j][self.idx_longitude_raw])
    ...
    def get_data(self):
        # ---- Cross-city support -------------------------------------------------
        # params['city_configs']: list of {'name', 'fin_temp', 'fin_original'}, one per city.
        # params['test_cities']: names (from city_configs) held out entirely as the unseen
        #   test city/cities. If empty/absent, falls back to the original single-city,
        #   date-based split for full backward compatibility.
        city_configs = self.params.get('city_configs', None)
        if city_configs is None:
            city_configs = [{'name': self.params.get('data_name', 'default_city'),
                              'fin_temp': self.params['fin_temp'],
                              'fin_original': self.params['fin_original']}]

        test_cities = set(self.params.get('test_cities', []))
        cross_city_mode = len(test_cities) > 0

        cities_to_load = city_configs
        if cross_city_mode:
            if self.mode == 'test':
                cities_to_load = [c for c in city_configs if c['name'] in test_cities]
            else:
                cities_to_load = [c for c in city_configs if c['name'] not in test_cities]
            if len(cities_to_load) == 0:
                raise RuntimeError(f"No cities available for mode='{self.mode}' given test_cities={test_cities}")

        df_list = []
        cou_df_list = []
        self.aoi_dict, self.aoi_feature, self.aoi_time_adj, self.aoi_fre_adj = {}, {}, {}, {}

        for city_cfg in cities_to_load:
            city, fin_temp, fin_original = city_cfg['name'], city_cfg['fin_temp'], city_cfg['fin_original']

            if os.path.exists(fin_temp):
                city_df = pd.read_csv(fin_temp + "/package_feature.csv", sep=',', encoding='utf-8')
                city_df_cou = pd.read_csv(fin_temp + "/courier_feature.csv", sep=',', encoding='utf-8')
            else:
                city_df, city_df_cou = pre_process(fin=fin_original, fout=fin_temp, city_name=city, is_test=self.params['is_test'])

            if 'city' not in city_df.columns:
                city_df['city'] = city
            if 'city' not in city_df_cou.columns:
                city_df_cou['city'] = city

            # courier_id is only guaranteed unique *within* one city's raw data; when
            # combining cities, prefix it so trajectories from different cities never
            # get merged together by split_trajectory().
            city_df['courier_id'] = city_df['city'].astype(str) + '_' + city_df['courier_id'].astype(str)
            city_df_cou['id'] = city_df_cou['city'].astype(str) + '_' + city_df_cou['id'].astype(str)

            aoi_data_path = fin_temp + "/aoi_feature.npy"
            aoi_data = np.load(aoi_data_path, allow_pickle=True).item()
            self.aoi_dict[city] = aoi_data["aoi_dict"]
            self.aoi_feature[city] = aoi_data["aoi_feature"]
            self.aoi_time_adj[city] = aoi_data["aoi_time_adj"]
            self.aoi_fre_adj[city] = aoi_data["aoi_frequency_adj"]

            df_list.append(city_df)
            cou_df_list.append(city_df_cou)

        df = pd.concat(df_list, ignore_index=True)
        self.df_cou = pd.concat(cou_df_list, ignore_index=True)
        ...
        # Use city-centered coordinates as model input (see preprocess_delivery.py),
        # not raw lat/lng, since raw coordinates don't transfer across cities.
        self.idx_longitude = idx(df, 'lng_norm')
        self.idx_latitude = idx(df, 'lat_norm')
        # Raw coordinates are still needed for geodesic distance calculations (self.dis_cal)
        self.idx_longitude_raw = idx(df, 'lng')
        self.idx_latitude_raw = idx(df, 'lat')
        ...
        if cross_city_mode:
            # `cities_to_load` (and thus `df`) is already restricted to the right
            # city set for this mode via test_cities above. Within the train-city
            # pool, carve out a val split by date, done *per city* so no city's
            # early dates leak into 'train' while its late dates go to 'val' of
            # a different city's date range.
            if self.mode in ('train', 'val') and not self.params['is_test']:
                keep_frames = []
                for city in df['city'].unique():
                    city_mask = df['city'] == city
                    city_dates = sorted(df.loc[city_mask, 'ds'].unique())
                    split_at = int(len(city_dates) * self.params['train_ratio'])
                    wanted_dates = set(city_dates[:split_at]) if self.mode == 'train' else set(city_dates[split_at:])
                    keep_frames.append(df[city_mask & df['ds'].isin(wanted_dates)])
                df = pd.concat(keep_frames, ignore_index=True)
            # mode == 'test': already scoped to held-out cities above, use all their dates
            # (this is the unseen-city evaluation set).
        else:
            # Original single-city, date-based split (unchanged behavior).
        ...
        # Re-sort after concatenation/filtering so split_trajectory() sees each
        # (courier, day) block as contiguous rows again.
        df = df.sort_values(by=['ds', 'courier_id', 'finish_time_minute']).reset_index(drop=True)
...
def get_params():
    ...
    # --- Cross-city options ---
    # If you only pass fin_temp/fin_original/data_name above, behavior is unchanged
    # (single city, date-based train/val/test split).
    # For cross-city (train on N-1 cities, test on 1 unseen city), set city_configs
    # programmatically before calling DeliveryDataset(params), e.g.:
    #
    #   cities = ['yt', 'cq', 'hz', 'sh', 'jl']
    #   params['city_configs'] = [
    #       {'name': c, 'fin_temp': ws + f'/data/tmp/delivery_{c}/',
    #        'fin_original': ws + f'/data/raw_data/delivery/delivery_{c}.csv'}
    #       for c in cities
    #   ]
    #   params['test_cities'] = ['jl']  # held-out unseen city; the rest are used for train/val
    parser.add_argument('--test_cities', type=str, nargs='+', default=[],
                         help='city names (matching city_configs entries) held out entirely as the unseen test city/cities')
